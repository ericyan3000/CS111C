...
public class HomeworkM10FolderDriver {
	...
	public static void main(String[] args) {
    	...
    	Folder folder2a1=new Folder("Folder2a1");
      	folder2a.addFolder(folder2a1);
    	Folder folder4b1=new Folder("Folder4b1");
    	folder4b.addFolder(folder4b1);
    	Folder folder4b2=new Folder("Folder4b2");
    	folder4b.addFolder(folder4b2);
    	...
    	System.out.println("\nTest that list order is not altered:");
    	System.out.println("Folder list is \t[Folder1, Folder2, Folder3, Folder4]");
    	System.out.println("\t\t"+topFolder.getFolderList());
	...
	public static void printFolderContentsWithStack(Folder topFolder) {
		...
		while(!folderStack.isEmpty()) {
			...
			int indentCount = indentStack.pop();
			...
			// print all the files
			if (currentFolder.getFileList().size() > 0) {
			...
			// print all the subFolders by pushing them into the stack
			int nextIndent = ++indentCount;
			for (int i = currentFolder.getFolderList().size() - 1; i >= 0; i--) {
				folderStack.push(currentFolder.getFolderList().get(i));
