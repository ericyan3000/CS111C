...
public class HomeworkM8Driver {
	...
	public static double sortedness(Comparable[] array) {
		double totalPair = array.length - 1;
		int sortCount = 0;
	...
	public static double sortedness(Node<Comparable> node) {
		if (node == null || node.next == null)
			...
			return sortednessHelper(node, 0.0, 0);
	}
	
	private static double sortednessHelper(Node<Comparable> node, double totalPair, int sortedCount) {
		if (node.next == null)
			return sortedCount / totalPair ;
		else 
			if (node.data.compareTo(node.next.data) <= 0)
				sortedCount++;
			totalPair++;
			return sortednessHelper(node.next, totalPair, sortedCount);
	}
