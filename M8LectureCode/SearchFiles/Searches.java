...
public class Searches {
	...
	public static ArrayList<Integer> findIndicesRecursiveSorted(Comparable[] array, Comparable target) {
		if (array.length == 0) 
			...
			return findIndicesRecursiveSortedHelper(array, tempList, 0, array.length - 1, target);
		}
	}
	
	private static ArrayList<Integer> findIndicesRecursiveSortedHelper(Comparable[] array, ArrayList<Integer> tempList, int begin, int last, Comparable target) {
		if (array[begin].equals(target))
			tempList.add(begin);
		if (begin >= last)
			return tempList;
		else 
			return findIndicesRecursiveSortedHelper(array, tempList, begin+1, last, target);
	}
