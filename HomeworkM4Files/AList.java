...
public class AList<T extends Comparable<? super T>> implements ListInterface<T>, Comparable<AList<T>> {
	...
	public int removeAll(T element) {
		if (numberOfEntries == 0) {
			...
			T[] newList = (T[]) new Comparable[numberOfEntries+1]; // create a new array
			int newNumberOfEntries = 0;
			for (int i = 1; i <= numberOfEntries; i++) {
				if (!list[i].equals(element)) { // put non-match element to the new array
					newNumberOfEntries++;
					newList[newNumberOfEntries] = list[i];
				}
				else
					count++; // increment removed count
			}
			this.list = newList; // assign the list to the newList reference
			this.numberOfEntries = newNumberOfEntries; // update numberOfEntries to match the newList
			return count;
		}
	}
	...
	public boolean equals(Object other) {
		if (other instanceof AList<?>) {
			...
			if (getLength() != otherList.getLength()) // lists not having the same number of elements
				return false;
			else {
				for (int i = 1; i <= getLength(); i++) {
					if (!list[i].equals(otherList.getEntry(i))) // not the same element at the same position
						return false;
				}
			}
			return true;
	...
	public int compareTo(AList<T> otherList) {
		 if (getLength() != otherList.getLength()) { // not the same length
			 return getLength() - otherList.getLength();  // return the difference of the lengths
		 }
		 else {
			 for (int i = 1; i <= getLength(); i++) {
				 if (!(getEntry(i)).equals(otherList.getEntry(i))) { // not the same element
					 return (getEntry(i)).compareTo(otherList.getEntry(i));
				 }
			 }
