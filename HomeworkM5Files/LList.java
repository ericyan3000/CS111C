...
public class LList<T extends Comparable<? super T>> implements ListInterface<T> {
	...
	public void removeAll(T item) {
		if (!isEmpty()) {
			if (numberOfEntries == 1) { // special case for singleton list
				...
				while (firstNode != null && firstNode.data.equals(item)) { // remove from the beginning 		
					firstNode = firstNode.next;
					numberOfEntries--;
				}
	...
	public T getMax() {
		if (isEmpty()) 
			...
			while (currentNode != null) {
				if (max.compareTo(currentNode.data) < 0)  // if max is less than current node data
	...
	public void addAll(T[] items) {
		...
		for (T item : items) {
			if (lastNode == null ) {
				firstNode = new Node(item); // update first node if it's first entry
				lastNode = firstNode;
			}
			else {
				Node newNode = new Node(item); // create a new node
				lastNode.next = newNode; // linked the new node after last node
				lastNode = newNode; // update last node to the new node
			}
			numberOfEntries++;
		}
	}
