...
public final class ArrayQueue<T> implements QueueInterface<T> {
	...
	public void splice(ArrayQueue<T> secondQueue) {
		// traverse the secondQueue by index
		// add each element to the current queue by enqueue
		int secondFront = secondQueue.frontIndex;
		int secondBack = secondQueue.backIndex;
		if (!secondQueue.isEmpty()) {
			for (int i = secondFront; i != secondBack; i = (i+1) % secondQueue.queue.length) {
				this.enqueue(secondQueue.queue[i]);
			}
			this.enqueue(secondQueue.queue[secondBack]);
		}
	}
