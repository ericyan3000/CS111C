...
public class BinaryTree<T> implements BinaryTreeInterface<T> {
	...
	public boolean containsAsLeaf(T entry) {
		BinaryNode<T> entryNode = new BinaryNode<T>(entry);
		return containsAsLeafHelper(root, entryNode);
	}
	
	private boolean containsAsLeafHelper(BinaryNode<T> currentNode, BinaryNode<T> node) {
		// if currentNode is null
		if (currentNode == null) 
			return false;
		// if the currentNode matches and isLeaf
		else if (currentNode.getData().equals(node.getData()) && isLeaf(currentNode))
			return true; 
		else 
			return containsAsLeafHelper(currentNode.getLeftChild(), node) || containsAsLeafHelper(currentNode.getRightChild(), node);
	}
	
	private boolean isLeaf(BinaryNode<T> currentNode) {
